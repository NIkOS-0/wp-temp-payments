{{-- Template Name: Single Service Card --}}
@extends('layouts.app')
...
@section('content')
  ...
  <style>
    ...
    .book-cover {
      width: 200px; height: 280px; background: linear-gradient(135deg, #10B981 0%, #047857 100%);
      box-shadow: -8px 8px 24px rgba(0,0,0,0.25), 4px 0px 0px rgba(0,0,0,0.1); border-radius: 12px;
      position: relative; display: flex; flex-direction: column; justify-content: flex-end; padding: 20px 16px;
    }
    .book-cover-text { font-weight: 800; font-size: 18px; color: #fff; text-shadow: 0 1px 8px rgba(0,0,0,0.2); line-height: 1.2; padding-left: 14px; text-align: center; margin-bottom: auto; margin-top: auto;}
    .book-cover-sub { font-weight: 400; font-size: 11px; color: rgba(255,255,255,0.8); padding-left: 14px; margin-top: 4px; text-align: center;}
    ...
    <div class="book-page">
      ...
      <section class="book-hero">
        <div class="book-gallery">
          <div class="book-gallery-main" id="book-gallery-main">
            @if($service['image_badge'])
              <span class="book-badge-bestseller">{{ $service['image_badge'] }}</span>
            @endif
            
            @if(!empty($service['gallery']))
              @foreach($service['gallery'] as $idx => $img)
            ...
            @else
              <div class="book-cover">
                <div class="book-cover-text">Консультация</div>
                @if($service['subtitle'])
                  <div class="book-cover-sub">{{ $service['subtitle'] }}</div>
                @endif
              </div>
            @endif
          </div>

          @if(!empty($service['gallery']) && count($service['gallery']) > 1)
        ...
        <div class="book-info">
          <div class="book-tags">
            @foreach($service['top_labels'] as $label)
              @php
          ...
          <h1 class="book-product-title">{!! nl2br(esc_html($service['title'])) !!}</h1>
          
          @if($author)
            <p class="book-author-line">Специалист: {{ $author['name'] }} · {{ $author['spec'] }}</p>
          @endif
          ...
          <div class="book-price-row">
            @if($service['price_old'])
              <span class="book-price-old">{{ $service['price_old'] }} ₽</span>
              @if($service['price'] && (int)$service['price'] > 0)
                <span class="book-price-badge">−{{ round(100 - ((float)$service['price'] / (float)$service['price_old']) * 100) }}%</span>
              @endif
            @endif
          </div>
          <div class="book-price-row" style="margin-bottom: 2px;">
            <span class="book-price-main">{{ $service['price'] ? $service['price'] . ' ₽' : 'Бесплатно' }}</span>
          </div>
          
          <div class="book-delivery-note">{{ $service['delivery_note'] }}</div>

          @php $buy_url = get_permalink(get_the_ID()); @endphp
          <div class="book-cta-row">
            @if(is_user_logged_in())
              <button class="book-btn-buy" onclick="window.location.href='{{ $buy_url }}'">Записаться</button>
            @else
              <button class="book-btn-buy" onclick="window.edietOpenBuyModal('{{ $buy_url }}')">Записаться</button>
            @endif
      ...
      <section class="book-tabs-card">
        ...
        <div class="book-tab-content active" id="tab-desc">
          <h2 class="book-desc-heading">О консультации</h2>
          <div class="book-desc-paragraph">
            @if(empty($service['description']))
               <p>Подробное описание консультации поможет вам лучше понять, как проходит прием и какие результаты вы получите.</p>
            @endif
