...
<footer id="site-footer" class="ediet-footer" role="contentinfo">
  ...
  <div class="ediet-footer__main">
    ...
    <div class="container-editorial relative z-10" style="padding-top:72px; padding-bottom:36px;">
      ...
      {{-- ══ CONTACTS + SOCIAL + PAYMENT ══ --}}
      <div class="divider"></div>
      <div class="ediet-middle-grid" style="display:grid; gap:40px; padding:48px 0; grid-template-columns: 1fr;">

        {{-- Contacts --}}
        @if(!empty($contacts_items))
        <div>
          <h4 class="section-heading">{{ $contacts_title }}</h4>
          <div style="display:grid; gap:10px;">
            @foreach($contacts_items as $c)
              @php $href = $c['url'] ?: '#'; @endphp
              <a href="{{ $href }}" class="ediet-contact-card" @if(!empty($c['url'])) target="_blank" rel="noopener" @endif>
                <span class="ediet-contact-card__icon">{!! $contact_icon($c['type'] ?? 'email') !!}</span>
                <span>
                  <span class="ediet-contact-card__title">{{ $c['title'] ?? '' }}</span>
                  @if(!empty($c['subtitle']))
                    <span class="ediet-contact-card__sub">{{ $c['subtitle'] }}</span>
                  @endif
                </span>
              </a>
            @endforeach
        ...
        <div>
          <h4 class="section-heading">{{ $social_title }}</h4>
          <div style="display:flex; flex-wrap:wrap; gap:10px;">
            @foreach($social_items as $s)
              @php
                $type = $s['type'] ?? 'custom';
                $url  = $s['url']  ?? '#';
                $lbl  = $s['label'] ?? ucfirst($type);
              @endphp
              <a href="{{ $url }}" class="ediet-social-pill" target="_blank" rel="noopener" aria-label="{{ $lbl }}">
                <span class="ediet-social-pill__icon">
                  @if($type === 'custom' && !empty($s['custom_icon']))
                    <img src="{{ $s['custom_icon'] }}" alt="">
                  @else
                    {!! $social_icon($type) !!}
                  @endif
                </span>
        ...
        {{-- Payment & Currency --}}
        @if(!empty($payment_items) || $security_note)
        <div>
          <h4 class="section-heading">{{ $payment_title }}</h4>

          @if(!empty($payment_items))
            <div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:16px;">
              @foreach($payment_items as $p)
                <span class="ediet-pay-chip">
                  @if(!empty($p['logo']))<img src="{{ $p['logo'] }}" alt="">@endif
                  <span>{{ $p['label'] ?? '' }}</span>
                </span>
              @endforeach
            </div>
          ...
          <div style="margin-bottom:16px;">
            <div style="font-size:11px; font-weight:700; color:rgba(168,159,139,0.7); letter-spacing:0.15em; text-transform:uppercase; margin-bottom:8px;">Валюта</div>
            <span class="ediet-currency-chip">₽ RUB</span>
          </div>

          @if($security_note)
            <div style="display:flex; gap:10px; align-items:flex-start; padding:14px; border:1px solid rgba(245,239,226,0.07); border-radius:12px; background:rgba(245,239,226,0.03);">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#d4a94a" stroke-width="1.8" style="flex-shrink:0; margin-top:2px;">
                <path d="M12 2 4 6v6c0 5 3.5 9.2 8 10 4.5-.8 8-5 8-10V6l-8-4z"/>
              </svg>
              <p style="color:rgba(245,239,226,0.65); font-size:12px; line-height:1.6; margin:0;">{!! nl2br(esc_html($security_note)) !!}</p>
            </div>
          @endif
        </div>
        @endif
