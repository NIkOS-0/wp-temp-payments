<a href="{{ $item['url'] }}" class="book-product-card" style="position: relative;">
  <div class="book-card-img" style="{{ !empty($item['image']) ? 'background-image: url('.$item['image'].'); background-size: cover; background-position: center;' : '' }}">
    @if(empty($item['image']))
      ...
      <div class="book-card-book" @if($item['type_label'] === 'Курс') style="background: linear-gradient(135deg, #10B981 0%, #047857 100%)" @endif>
  ...
  <div class="book-card-body">
    @if($item['type_label'] === 'Курс')
      <div style="font-size:10px; font-weight:700; color:#10B981; letter-spacing:0.1em; text-transform:uppercase; margin-bottom:4px; margin-top: 14px;">Курс</div>
    @endif
    <h3 style="font-size: 14px; font-weight: 700; margin-bottom: {{ $item['type_label'] === 'Курс' ? '4px' : '12px' }}; line-height: 1.35;{{ $item['type_label'] !== 'Курс' ? 'margin-top: 14px;' : '' }}">{{ $item['title'] }}</h3>
